...
const Qub = () => {
  ...
  return (
      <div className="bg-white p-8  max-w-full">
        <h2 className="text-2xl font-bold mb-4">2. How is this home cooled?(choose the main system, if more then one)</h2>
        <div className={`space-y-4 grid-flow-rows grid-cols-2 gap-2 md:grid ${showShakeAnimation ? 'shake 0.8s ease-in-out' : 'none'}`}>
          {['Control AC with room vents', 'Room ACs', "Doesn't have Cooling", "Don't know/Unsure"].map((answer, index) => (
            <button
              key={index}
        ...
        <div className='flex justify-between'>
          ...
          <div className='pt-4'>
          <Paginate prevDirection="up" prevTargetPage="/Qua" nextDirection="down" nextTargetPage="/Quc" />
          </div>
        </div>
      </div>
  );
